...
#' Format Database Breakdown Dataframe V2
#'
...
#'
formatDF_2 <- function(df, dateColName, total_seqs, total_seqs_1m, oneMonth = FALSE) {
...
#' Make Database Breakdown Dataframe V2
#'
...
#'
makeDFdatabaseBreakdown_2 <- function(DB, interval, lastMonth, total_seqs, total_seqs_1m) {
  ...
  df <- formatDF_2(df, interval, total_seqs, total_seqs_1m)
  ...
  df_lastMonth <- formatDF_2(df_lastMonth, lastMonth, total_seqs, total_seqs_1m, oneMonth = TRUE)
...
#' Make Database Breakdown Table (Condensed Version)
#'
...
#' @export
#'
makedbBreakdownTableCondensed <- function(DB, variantFile, interval, lastMonth, colWidth, extraNote = NULL, saveTable = FALSE, saveFolder = NULL, height = NULL, width = NULL) {
  setStrings2AssayDB()
  ...
  df <- makeDFdatabaseBreakdown_2(DB, interval, lastMonth, total_seqs, total_seqs_1m)
  ...
  # Add footnotes explaining count(percentage) formatting and recombinant lineages
  gt_tbl <- gt_tbl %>%
    gt::tab_footnote(
  ...
  if (saveTable) {
    ...
    gt_tbl %>% gt::gtsave(paste0(saveFolder, "database_breakdown.png"), vwidth = width)
  }
}
