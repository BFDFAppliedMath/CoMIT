...
v <- create_test_fasta_and_DB_amplicons()
...
# tests:
# if function that adds stuff to db works
# if right stuff is added to db
# look at last 2 tests in test-helper-key_comps
...
# test_that("Amplicon Table Counts", {
#     DB <- testEnv$TESTDB
#     mutKey <- testEnv$mutationKey
#
#     con <- DBI::dbConnect(RSQLite::SQLite(), DB)
#     ampliconInfo <- dplyr::tbl(con, s$AMPLICON_TABLE) %>% dplyr::collect()
#     DBI::dbDisconnect(con)
#
#     mutKey <- mutKey %>% dplyr::group_by() # unique id
#
#     # paste0("EPI_ISL_",seq))
#
#     #Check that sequences were added
#     expect_equal(nrow(ampliconInfo), 7)
# })
...
#   totalseqs <- mutKey %>% dplyr::group_by(Seq_Num) %>%
#     dplyr::summarise(n = dplyr::n()) %>% nrow()
...
#     dplyr::summarise(n = dplyr::n(), .groups = 'drop') %>% dplyr::filter(n == 2)
#
...
#   con <- DBI::dbConnect(RSQLite::SQLite(), DB)
#   #Tables with Needed Information
#   varList <- dplyr::tbl(con, s$VAR_ACC_ID_MAP_TABLE) %>% dplyr::collect()
#   DBI::dbDisconnect(con)
#
#
#   #Invidivual Row check------------------------------------------------------------------------
#   #Check Matches (if in the variation list then not classified as match)
#   matches = mutKey %>% dplyr::filter(M_V == "M" | M_V == "MC") %>%
#     dplyr::select(Seq_Num, Primer_Num)
#   for(row in 1:nrow(matches)) {
#     varCount = varList %>% dplyr::filter(get(s$ACC_ID)== matches$Seq_Num[row]) %>%
...
#
#     varKey = mutKey %>% dplyr::filter(Seq_Num == aNum) %>%
...
#
#     if ((isMut == "V") | (isMut == "VC")){
#       varTotal = varList %>% dplyr::filter(get(s$ACC_ID) == paste0("EPI_ISL_", seq)) %>%
...
#     if(isMut == "A"){
#
#       ambPrimer = ambTable %>% dplyr::filter(get(s$ACC_ID) == paste0("EPI_ISL_",seq)) %>%
#         dplyr::filter(get(s$PRIMER_ID) == currPrimer)
#
...
#     #Get Key
#     varCount = mutKey %>% dplyr::filter(Seq_Num == seqR) %>%
#       dplyr::filter(M_V == "V" | M_V == "VC")
#     doubleCount = varCount %>% dplyr::group_by(Seq_Num, Primer_Num) %>%
